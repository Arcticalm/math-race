...
运行：` .venv/bin/python -m final_code.problem2.run --output outputs/q2 --time-limit 120 `。输出包含 `problem2_submission.xlsx`、逐箱与资源审计、路线图和 `validation.json`。现有最终方案在 `outputs/q2/`。

CP-SAT 回代后按固定无人机及电池先后关系执行连续紧排；物理、时限和资源独立复核通过后才导出。
