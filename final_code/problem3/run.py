...
from final_code.problem3.transport_relay import _plot_problem3_overview, build_trajectory
from final_code.run_all import audit_and_export_joint


def run(output: Path, time_limit: float = 120.0, max_relays: int = 10) -> dict:
    ...
    try:
        ...
        if q2 is None:
            raise RuntimeError(f"No transport warm start within limits: {q2_search['status']}")
        ...
        write_program_bundle(output / "problem3_program.zip")
        return metrics
    finally:
        evaluator.close()
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "outputs/problem3/final_code")
    parser.add_argument("--time-limit", type=float, default=120.0)
    parser.add_argument("--max-relays", type=int, default=10)
    args = parser.parse_args()
    if args.time_limit <= 0 or args.max_relays < 0:
        parser.error("time-limit must be positive and max-relays nonnegative")
    result = run(args.output, args.time_limit, args.max_relays)
