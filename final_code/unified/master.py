...
def solve(pool, boxes, drones, batteries, evaluator, time_limit=60.,
    ...
    pattern and resource from the same pool. Its three-group labels guarantee
    that subsequent frozen Q4 partitions exist under the shared-relay closure
    convention. Group labels do not prevent resource sharing during Q3.
    """
    if not pool or not boxes:
        raise ValueError("Empty pattern library or task set")
    if time_limit <= 0 or horizon <= 0 or max_relays < 0:
    ...
    if joint:
        ...
        if len(sites) < 3:
            raise ValueError("Q3/Q4 requires at least three service areas")
        for site in sites:
            site_groups[site] = model.new_int_var(0, 2, "partition_" + site)
    ...
    if joint:
        ...
        if requirements and not locations:
            return None, {"status": "INFEASIBLE", "scope": "no relay candidate locations"}
        ...
        for slot in range(max_relays if requirements else 0):
            use = model.new_bool_var(f"relay_{slot}")
            loc = model.new_int_var(0, len(locations) - 1, f"location_{slot}")
            group = model.new_int_var(0, 2, f"relay_group_{slot}")
    ...
    info = {"time_grid_s": 1, "energy_grid_kwh": 1e-6,
            ...
            "three_group_compatibility_required": joint, "stages": []}
    incumbent = None
