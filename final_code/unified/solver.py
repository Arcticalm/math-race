...
from src.problem23.terrain_audit import audit_clearance
from src.problem3.audit import audit_communication, audit_relay_resources
from src.problem3.physics import LinkEvaluator, load_relay_parameters
...
from src.problem4.solver import run as partition_frozen_schedule
...
def write_program_bundle(output):
    ...
    paths += [ROOT / "tests/test_unified.py", ROOT / "tests/test_problem4.py",
              ROOT / "tests/test_problem3_timeline.py", ROOT / "tests/__init__.py",
              ROOT / "docs/结果提交模板.xlsx"]
    with zipfile.ZipFile(output / "unified_program.zip", "w", zipfile.ZIP_DEFLATED) as archive:
