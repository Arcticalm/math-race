...
DEFAULT_RESERVE_SENSITIVITY = (0.10, 0.15, 0.25, 0.30, 0.40, 0.50)
DEFAULT_ENERGY_SENSITIVITY = (0.8, 1.2)
...
def run(
    ...
    reserve_levels.extend(float(reserve) for reserve in (
        DEFAULT_RESERVE_SENSITIVITY if reserves is None else reserves
    ))
    reserve_levels = list(dict.fromkeys(round(value, 10) for value in reserve_levels))
    if any(not math.isfinite(reserve) or not 0 <= reserve <= 1 for reserve in reserve_levels):
        raise ValueError("Reserve fractions must be finite and between 0 and 1")
    energy_scale_levels = [1.0]
    energy_scale_levels.extend(float(scale) for scale in (
        DEFAULT_ENERGY_SENSITIVITY if energy_scales is None else energy_scales
    ))
    energy_scale_levels = list(dict.fromkeys(round(value, 10) for value in energy_scale_levels))
    if any(not math.isfinite(scale) or scale <= 0 for scale in energy_scale_levels):
        raise ValueError("Horizontal energy scale factors must be finite and positive")
        ...
        for reserve, energy_scale in scenarios:
            ...
            for site_code, site in sites.items():
                ...
                for aircraft in aircrafts.values():
                    ...
                    empty_flight = flight_for_payload(
                        active, energy_rates[active.code], base, site, max_elevation, 0.0
                    )
                    payload_rows.append({
                        ...
                        "empty_round_trip_energy_kwh": empty_flight.energy_kwh,
                        "max_payload_round_trip_energy_kwh": flight.energy_kwh,
                        "empty_round_trip_feasible": empty_flight.energy_kwh <= active.usable_energy_kwh * (1 - reserve) + 1e-12,
                    })
            ...
            primary = policy_partitions.get("架次优先", {})
            solution_rows = []
...
def write_outputs(result: dict, output_dir: Path) -> None:
    ...
    with (output_dir / "safe_payloads_sensitivity.csv").open("w", encoding="utf-8-sig", newline="") as stream:
        fields = ["reserve_fraction", "horizontal_energy_scale", "site", "aircraft", "max_safe_payload_kg", "route_distance_m", "cruise_altitude_m", "empty_round_trip_energy_kwh", "max_payload_round_trip_energy_kwh", "empty_round_trip_feasible"]
        writer = csv.DictWriter(stream, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for item in result["runs"]:
...
def _plot_sensitivity(runs: list[dict], output_path: Path) -> None:
    ...
    times = [item["summary"]["total_work_time_s"] / 3600
             if item["summary"]["total_work_time_s"] is not None else float("nan")
             for item in runs]
...
def _plot_energy_sensitivity(runs: list[dict], output_path: Path) -> None:
    runs = sorted(
        (item for item in runs if item["reserve_source"] == "aircraft_workbook_default"),
