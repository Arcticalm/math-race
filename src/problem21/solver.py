...
def _validate(
    ...
    for sortie in sorties:
        ...
        if abs(sortie.prep_end_s - (sortie.prep_start_s + aircraft.prep_s)) > 1e-7:
            ...
            violations.append(message)
            physics_violations.append(message)
        if abs(sortie.takeoff_s - (sortie.prep_end_s + aircraft.load_each_s * len(sortie.boxes))) > 1e-7:
    ...
    for battery_code, values in battery_assignments.items():
        ...
        for previous, current in zip(values, values[1:]):
            soc = 1 - previous.energy_kwh / load_aircraft()[battery.aircraft].usable_energy_kwh
...
def _write_program_bundle(output_path: Path) -> None:
    ...
    include = [
        ROOT / "src/problem1/__init__.py",
        ROOT / "src/problem1/solver.py",
        ROOT / "src/problem1/requirements.txt",
        ROOT / "src/problem2/__init__.py",
        ROOT / "src/problem2/solver.py",
        ROOT / "src/problem2/requirements.txt",
        ROOT / "src/problem2/README.md",
...
def write_outputs(
    ...
    (output_dir / "model_assumptions.json").write_text(json.dumps({
        ...
        "dem_sampling": "coordinate-linear path sampled at <=30 m; maximum valid raster elevation",
