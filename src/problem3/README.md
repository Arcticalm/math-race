...
`--time-limit` 是每轮后移协调的 CP-SAT 求解秒数（默认 180 秒）；固定时间轴上的中继选择
...
- 后移协调阶段允许连续相交的需求链，不再要求所有需求共享同一时刻。
  重叠前驱按原需求编号定向，合并后没有无需求的等待间隙；这仍是有限候选
...
可运行 `python -m src.problem3.independent` 从原始货箱、路线、无人机、电池和地形输入
重新构造并排程问题三，不读取 `outputs/problem23`。目前独立可行结果位于
`outputs/problem3/independent/`：25 个运输架次、7 个中继任务，总完成时间
10192.567 秒，比上方问题二时间轴方案少 225.089 秒；46 项硬时限通过，连续通信
中断 0 秒，运输及中继资源无冲突。实现假设、运行方式和有限候选最优性边界见
