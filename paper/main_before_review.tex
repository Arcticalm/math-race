...
\newcommand{\PaperKeywords}{应急物资运输\quad 无人机组批\quad 空气动力学非线性能耗\quad 最大安全载荷\quad 集合划分模型\quad 状态压缩动态规划\quad 帕累托优化\quad 敏感性分析}
...
% 取消 format.cls 自带的自动章节编号前缀，完美保留用户原稿中的大纲标题编号（如“一、”、“1.1”、“5.1.1”）
\makeatletter
\def\@seccntformat#1{}
\makeatother
...
\subsubsection{\texorpdfstring{5.1.2 载荷--等效航程 $3/2$ 次方动力学衰减模型}{5.1.2 载荷--等效航程 3/2 次方动力学衰减模型}}
...
\subsubsection{\texorpdfstring{5.2.3 $15\times 3$ 最大安全载荷计算结果与瓶颈演变机理}{5.2.3 15x3 最大安全载荷计算结果与瓶颈演变机理}}
...
\subsubsection{\texorpdfstring{5.6.1 返航安全余量 $\rho$ 单因素扰动分析}{5.6.1 返航安全余量 rho 单因素扰动分析}}
...
\toprule
\textbf{安全余量} $\rho$ & \textbf{总架次} & \textbf{总运输能耗}($\text{kWh}$) & \textbf{作业时间}($\text{s}$) & \textbf{可行状态} & \textbf{组批结构演化特征} \\
\midrule
...
$30\%$ & 20 & 67.349 & 36595.6 & 完全可行 & S002 与 S003 进一步分裂重组 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{\texorpdfstring{5.6.2 水平能耗标定因子 $\kappa$ 敏感性分析}{5.6.2 水平能耗标定因子 kappa 敏感性分析}}
...
{\kaishu （注：本节作为全文整体架构预留。问题二在问题一物理能耗与组批模型基础上，解除单点往返限制，建立带时间窗与充电周转的多旅行商车辆路径与资源排程联合优化模型，详见后续章节展开。）}
...
{\kaishu （注：本节作为全文整体架构预留。问题三在问题二运输调度方案基础上，全面引入无线信道自由空间损耗、衰落裕量及沿途 DEM 视距遮挡预算模型，协同规划空中中继无人机的三维悬停坐标与服务时序。）}
...
{\kaishu （注：本节作为全文整体架构预留。问题四以问题三固定的运输--通信联合调度方案为原子任务输入，研究 $K=2$ 与 $K=3$ 空间分区及独立执行单元的资源配置与库存缺口分析。）}
