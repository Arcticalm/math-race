...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{主选方案与物理周转机理解析}：最优方案出动 25 架次（8C+6B+11A，含 6 个双点串联航段），将全部任务完成时间大幅缩短至 $7983.81\,\text{s}$（约 $2$ 小时 $13$ 分 $04$ 秒，比 18 架次单点方案的完工时间提前 $1598\,\text{s}$，缩短 $16.7\%$）；80 件货箱实现 $100\%$ 无遗漏交付，46 项硬时限合规率达 $100\%$，总准时率达 $90.0\%$，总能耗为 $74.628\,\text{kWh}$。执行的全部 56 个飞行航段（35 条不同有向航段）经独立拓扑相交审计，最小安全净空严格达到 $50.000\,\text{m}$，净空违规数为 0。深度揭示了适度增加架次能够有效激活异构机队并发能力、缓解重载机型与电池周转瓶颈的内在工程规律。
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{巡航海拔与 50\,m 净空边界}：根据赛题规定，无人机在航段上的巡航海拔必须高出沿途经过地面的最高海拔至少 $50\,\text{m}$。在早期的离散采样计算中，若沿大圆航线按固定步长（如 $\le 30\,\text{m}$）等间距离散取样，可能会漏掉航线短暂掠过或角点相切的高地像元，从而造成微观净空不足。为此，本文建立了基于几何线段与栅格闭矩形拓扑相交的精确提取算法：
    将航段投影为经纬度几何线段，严格求出该线段与 30\,m 分辨率 DEM 栅格网格的所有交点，识别其经过并触碰的全部候选像元闭矩形（涵盖内部穿透、边界相交与角点接触）。设沿途经过的像元集合为 $\mathcal{C}_{v_k, v_{k+1}}$，则航段最高地表海拔定义为该集合内全部像元的高程极大值：
    \begin{equation}
    H_{v_k, v_{k+1}}^{\max} = \max_{c \in \mathcal{C}_{v_k, v_{k+1}}} \text{DEM}(c)
    \end{equation}
    巡航海拔据此设定为：
    \begin{equation}
    H_{pk}^{cruise} = H_{v_k, v_{k+1}}^{\max} + 50\,\text{m}
    \end{equation}
    例如，对于基地至服务区 $S007$ 的航段 $O01 \to S007$，早期等距采样识别的最高地形为 $451.83\,\text{m}$（对应巡航高度 $501.83\,\text{m}$），而精确线段—栅格拓扑相交算法成功捕获到航线斜切的一处山脊山头，其实际最高地形达 $461.97\,\text{m}$。因此该航段巡航海拔校正为 $511.97\,\text{m}$，从根本上杜绝了微观撞山风险，确保了 $50\,\text{m}$ 安全净空的绝对严格满足。
...
min\_sorties / cp\_sat & CP-SAT & 18 & 60.664 & 9581.4 & 362113.2 & 85.0\% & 能耗适中，但 C 型电池充能排队严重 \\
min\_sorties / heu\_balanced & 启发式平衡 & 18 & 60.664 & 10657.8 & 447130.1 & 87.5\% & 串行程度高，完工时间突破 2.9 小时 \\
min\_sorties / heu\_energy & 启发式节能 & 18 & 60.664 & 10657.8 & 447130.1 & 87.5\% & 与平衡启发式等价 \\
\midrule
\multicolumn{8}{l}{\textit{二、19架次方案（第一问能耗优先单点组批）}} \\
min\_energy / cp\_sat & CP-SAT & 19 & 61.314 & 8677.0 & 141886.2 & 88.8\% & 拆分 S008 缓解了部分压力，用时缩短 \\
min\_energy / heu\_balanced & 启发式平衡 & 19 & 60.566 & 9895.6 & 316237.9 & 87.5\% & 启发式调度下时间仍偏长 \\
min\_energy / heu\_energy & 启发式节能 & 19 & 60.566 & 9895.6 & 316237.9 & 87.5\% & 启发式节能基准解 \\
\midrule
\multicolumn{8}{l}{\textit{三、31架次方案（构造式轻量单点组批）}} \\
constructive / cp\_sat & CP-SAT & 31 & 82.924 & 8370.9 & 281712.2 & 88.8\% & 出动架次过多，空域起降频繁且电耗大 \\
constructive / heu\_balanced & 启发式平衡 & 31 & 81.838 & 10115.3 & 114740.7 & 88.8\% & 串行启发式效率偏低 \\
constructive / heu\_energy & 启发式节能 & 31 & 78.981 & 15298.3 & 311022.5 & 86.2\% & 为追求节能导致调度时间极端拖长 \\
\midrule
\multicolumn{8}{l}{\textit{四、25架次多点串联方案（构造式跨区合并组批，\textbf{本文主选方案}}）} \\
\textbf{merged / cp\_sat (主方案)} & \textbf{CP-SAT} & \textbf{25} & \textbf{74.628} & \textbf{7983.8} & \textbf{90818.0} & \textbf{90.0\%} & \textbf{完工时间最短(2h13m)，加权迟到最低} \\
merged / heu\_balanced & 启发式平衡 & 25 & 73.994 & 12576.7 & 313070.0 & 82.5\% & 启发式无法协调复杂多点航线冲突 \\
merged / heu\_energy & 启发式节能 & 25 & 73.994 & 12576.7 & 313070.0 & 82.5\% & 耗时达 3.49 小时，极度延误 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    ...
    在问题一中，18 架次方案在单点往返体制下极为优秀；然而在问题二考虑实体机与电池的实体排程中，18 架次方案的完工时间长达 $9581.4\,\text{s}$。深层根源在于：该方案将大量重载任务集中在重型 C 型机上（共安排 8 个 C 型架次）。但基地仅有 2 架 C 型实体机与 4 组电池，C 型电池充满一次需要整整 3600 秒（1小时）。这导致多架 C 型任务在时间轴上产生不可调和的强串行等待，基地出现“飞机等电池充电”的严重窝工。
    \item \textbf{适度多点拆解激活机群高频并发}：
    主选的 25 架次方案（\texttt{merged/cp\_sat}）通过挖掘 6 组跨区合并航线，适度将部分大载荷拆解为由 A 型与 B 型无人机承担的中轻型多点回路。虽然架次数增至 25，总能耗由 $60.664\,\text{kWh}$ 升至 $74.628\,\text{kWh}$，但由于充分释放了 4 架 A 型机（配 6 组电池、充能仅 30 分钟）和 2 架 B 型机的高频并发能力，彻底平抑了重载电池的充能等待。最终，全部任务完成时间由 $9581.4\,\text{s}$ 锐减至 \textbf{$7983.81\,\text{s}$（约 2 小时 13 分 04 秒）}，缩短了整整 $1598\,\text{s}$（降幅达 $16.7\%$）！
    \item \textbf{时效服务质量的压倒性飞跃}：
    主选方案在大幅压缩完工时间的同时，其应急优先级加权迟到量由 18 架次的 $362113.2$ 骤降至 \textbf{$90818.0$（降幅达 $74.9\%$）}，全网一般生活物资的按时交付率提升至 $90.0\%$（72/80 件按时送达），全部 46 项生命硬时限无一例外 $100\%$ 按时达标。这充分证明主方案兼具全局完成速度与民生保障时效的双重优势。
\end{enumerate}
...
Q23-002 & U07 & C & C-B01 & 0 & 540 & 1627.4 & S001 & 78.0 & 3.005 & 62.44 \\
Q23-003 & U08 & C & C-B02 & 0 & 420 & 1363.4 & S001 & 56.0 & 2.412 & 69.85 \\
Q23-004 & U02 & A & A-B05 & 0 & 360 & 2007.7 & S002 & 17.0 & 2.929 & 34.91 \\
Q23-005 & U08 & C & C-B02 & 3436 & 3916 & 5547.0 & S002 & 64.0 & 6.125 & 23.44 \\
Q23-006 & U02 & A & A-B05 & 3704 & 4064 & 5966.3 & S003 & 17.0 & 2.981 & 33.75 \\
Q23-007 & U08 & C & C-B04 & 5548 & 6028 & \textbf{7983.8} & S003 & 64.0 & 6.148 & 23.15 \\
Q23-008 & U06 & B & B-B02 & 5827 & 6187 & 7752.8 & S004 & 17.0 & 2.586 & 35.34 \\
Q23-009 & U07 & C & C-B03 & 5791 & 6211 & 7964.2 & S004 & 42.0 & 5.540 & 30.75 \\
Q23-010 & U02 & A & A-B03 & 2008 & 2368 & 3703.8 & S005 & 17.0 & 2.129 & 52.68 \\
Q23-011 & U07 & C & C-B04 & 1628 & 2048 & 3376.0 & S005 & 42.0 & 3.802 & 52.48 \\
Q23-012 & U03 & A & A-B02 & 2655 & 3015 & 3967.6 & S006 & 17.0 & 1.310 & 70.90 \\
Q23-013 & U04 & A & A-B02 & 4847 & 5207 & 7023.2 & S008 & 17.0 & 3.212 & 28.63 \\
Q23-014 & U08 & C & C-B03 & 1364 & 1754 & 3435.8 & S008 & 28.0 & 5.421 & 32.23 \\
Q23-015 & U05 & B & B-B04 & 6631 & 6991 & 7760.6 & S011 & 17.0 & 0.967 & 75.81 \\
Q23-016 & U01 & A & A-B01 & 0 & 330 & 1163.5 & S011 & 8.0 & 1.105 & 75.45 \\
Q23-017 & U05 & B & B-B03 & 0 & 360 & 1863.8 & S012 & 17.0 & 2.472 & 38.21 \\
Q23-018 & U01 & A & A-B01 & 2322 & 2682 & 4311.0 & S014 & 17.0 & 2.288 & 49.16 \\
Q23-019 & U04 & A & A-B06 & 2180 & 2540 & 4150.2 & S015 & 17.0 & 2.476 & 44.98 \\
Q23-020 & U01 & A & A-B01 & 5472 & 5832 & 7482.0 & S013 $\to$ S010 & 16.0 & 2.246 & 50.08 \\
Q23-021 & U06 & B & B-B02 & 0 & 390 & 2115.4 & S010 $\to$ S014 & 25.0 & 2.268 & 43.29 \\
Q23-022 & U06 & B & B-B04 & 2421 & 2811 & 4519.5 & S007 $\to$ S015 & 25.0 & 2.357 & 41.08 \\
Q23-023 & U05 & B & B-B01 & 2503 & 2893 & 4462.9 & S013 $\to$ S009 & 25.0 & 2.448 & 38.81 \\
Q23-024 & U05 & B & B-B03 & 4463 & 4853 & 6630.9 & S009 $\to$ S012 & 25.0 & 2.846 & 28.85 \\
Q23-025 & U07 & C & C-B01 & 3377 & 3887 & 5790.8 & S006 $\to$ S007 & 70.0 & 4.288 & 46.40 \\
\midrule
\textbf{全网汇总} & \textbf{8架全部出动} & \textbf{8C+6B+11A} & \textbf{14组全部周转} & -- & -- & \textbf{7983.8} & \textbf{含6个多点回路} & \textbf{704.0} & \textbf{74.628} & \textbf{均值48.2\%} \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{6.4.3 关键结果图表呈现与多维解读}
图 \ref{fig:routes_q2}、图 \ref{fig:clearance_q2}、图 \ref{fig:resource_gantt_q2}、图 \ref{fig:delivery_times_q2} 与 图 \ref{fig:tradeoff_q2} 分别从空间航路轨迹、三维地形净空核查、资源时序排程、物资交付阶梯及多目标权衡五个维度，全面呈现了最优协同调度方案的科学性与高可靠性。
...
\begin{figure}[H]
\centering
\SafeGraphic[width=0.88\textwidth]{terrain_clearance.png}
\caption{问题二各飞行航段最高地面高程与计划巡航高度对比图（50\,m 安全净空核查）}
\label{fig:clearance_q2}
\end{figure}

图 \ref{fig:clearance_q2} 直观展示了 25 个架次执行的全部 56 个飞行航段（对应 35 条不同有向航段）的最高地面海拔与计划巡航高度对比情况。由图可见，所有航段的柱状净空高度差均严格保持在 $50.000\,\text{m}$ 及以上（如最高山脊航段 $S006 \to S007$ 地面高达 $540.36\,\text{m}$，巡航海拔严格定为 $590.36\,\text{m}$；$O01 \to S014$ 地面高达 $541.95\,\text{m}$，巡航海拔定为 $591.95\,\text{m}$；$O01 \to S007$ 经栅格拓扑相交修正后地面达 $461.97\,\text{m}$，巡航海拔提升至 $511.97\,\text{m}$），全网无任何一处侵入 50\,m 安全净空包络，彻底消除了山区突起岩体对低空航行的碰撞隐患。
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{电池充放电梯次接力}：甘特图下半部分清晰揭示了共享电池的循环节拍。例如电池 C-B02 在执行完 Q23-003（$0\sim 1363.4\,\text{s}$，返航 SOC $69.85\%$）后立即接入充电机，依据式 (\ref{eq:charge_model}) 耗时 $1784.0\,\text{s}$ 于 $3147.4\,\text{s}$ 充至满电；随后在 $3436.0\,\text{s}$ 被重新装载至 U08 实体机执行 Q23-005。同理，C-B01、C-B03、C-B04 均经历了 2 次满负荷循环，周转间隔严丝合缝，无任何时段重叠。
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{航路净空审计（50\,m 安全净空绝对合规）}：调用独立核验工具（\texttt{terrain\_audit.py} 与 \texttt{terrain\_clearance\_audit.csv}），脱离求解器内部采样函数，直接计算各直线航段与 DEM 栅格闭矩形的几何拓扑相交，完整覆盖经过的全部像元。对主选方案执行的全部 56 个飞行航段（35 条不同有向航段）进行穿透式高程核验：所有航段的巡航海拔与沿线最高地表海拔之差均严格满足 $\ge 50.000\,\text{m}$，净空违规航段数为 0；
    \item \textbf{货箱覆盖审计}：80 件货箱唯一映射，分配率 $100\%$，无重复分配、无遗漏货箱；
    \item \textbf{硬时限达标审计}：46 项硬约束通过率 $100\%$，违约数为 0；
    \item \textbf{动力学与能耗合规审计}：25 个架次的返航剩余 SOC 均在 $23.15\% \sim 75.81\%$ 之间（最低为 Q23-007 的 $23.15\%$，最高为 Q23-015 的 $75.81\%$），全部严格高于 $20.0\%$ 法定安全底线；
...
需要向评审专家客观声明的是：在所选固定组批拓扑下，Google OR-Tools CP-SAT 求解器给出了首阶段整秒离散模型的 \texttt{OPTIMAL} 最优性证明证书，模型完工时间与其对偶下界严格重合于 $7984\,\text{s}$，经原高精度浮点动力学独立重放审计后的真实完工时刻为 $7983.81\,\text{s}$。但这属于固定候选拓扑与离散秒级网格下的\textbf{条件最优解}，并不构成未受限全拓扑空间下的全局数学最优性证明。若需进一步寻求全局最优性理论证书，后续可基于分支定价（Branch-and-Price）框架构建大规模列生成算法，将连续多点航线生成作为带资源约束的最短路定价子问题（ESPPRC），在主问题与子问题之间迭代生成列，进而给出全局对偶下界与最优性间隙（Optimality Gap）。然而，对于极端山区抢险救灾而言，本文算法在数十秒内给出的 $7983.8\,\text{s}$ 严谨可行解，已展现出卓越的工程响应力与实用价值。
