...
\begin{enumerate}[leftmargin=*]
    \item \textbf{协同排程建模}。建立了以全部任务完成时间为首要目标、非医疗货箱加权迟到量为次要目标的混合整数规划模型。模型刻画了航路沿线最高地面高程加 50 m 的巡航海拔标定、多点架次逐段递减的载荷与能耗、每点交付后从 30 m 作业高度重新爬升、两阶段等效充电时间折算、46 项医疗与首批保障硬时限，以及实体机与共享电池的区间不重叠约束。

    \item \textbf{两阶段求解}。先用逐区状态压缩动态规划生成 18、19、31 架次三种单点组批，再用贪心两区邻域合并把 31 架次压缩为 25 架次的多点方案；对每种固定拓扑用 Google OR-Tools CP-SAT 做整秒离散排程，共比较 12 组候选方案。在 CP-SAT 中同一条航线的机型可在排程阶段重新选择，因此不同排程方法对同一组批会给出不同能耗。

    \item \textbf{主选方案}。25 架次方案（8C+7B+10A，含 6 条双点回路）把完工时间压到 $7983.81\,\text{s}$，比 18 架次单点方案缩短 $1597.6\,\text{s}$（$16.7\%$）；总能耗 $74.628\,\text{kWh}$，80 件货箱全部交付，46 项硬时限全部达标，80 件货箱的期望时间准时率 $90.0\%$。全部 56 个飞行航段（35 条有向航段）经独立几何审计，最小净空 $50.000\,\text{m}$，违规数为 0。此外，机队利用率并不均衡：U03 全程只执行 1 个架次，A 型 6 组电池中 A-B04 未被使用。
\end{enumerate}
...
问题一里每个架次只服务一个服务区，15 个服务区互相独立，组批只需看本点的载荷、体积与电量边界。这样处理有两个代价：一是运力浪费，S009$\sim$S015 这类点单点物资只有 $25\,\text{kg}$，单独飞一个往返，航程和装载能力都用不满；二是问题一没有考虑实体机和电池属于有限资源，同一时刻能同时出动的架次数量实际上受机队和电池库存限制\cite{poikonen2017, schermer2019}。

问题二取消单点往返限制，允许一个架次连续访问多个服务区，并把实体机、共享电池和物资时限一并纳入。相比问题一，新增的内容有四项。

\begin{enumerate}[leftmargin=*]
    \item \textbf{多点串联与载荷沿途递减}。一个架次可以按 $O01 \to S_i \to S_j \to O01$ 的顺序访问多个服务区。每交付一个点，机上剩余载荷就减少一部分，后续航段的等效航程和能耗都要按新的载荷重算；每到一个点完成交付后，还要从该点的 $30\,\text{m}$ 作业高度重新爬升到下一段的巡航海拔。
    \item \textbf{实体无人机数量有限}。基地只有 8 架运输无人机，同一架飞机在同一时刻只能执行一个架次。
    \item \textbf{共享电池需要充电周转}。基地共 14 组共享电池，同机型的电池可以在该机型的飞机之间轮换，不同机型不可混用；电池返航后必须充至 $100\%$ 才能再次装机。
    \item \textbf{物资时限}。80 件货箱中有一部分带硬时限，其余带期望送达时间，用于衡量配送及时性。
\end{enumerate}

\subsubsection{6.1.2 实体机与共享电池配置}
...
\centering
\small
\caption{前线临时调度中心 $O01$ 实体运输无人机与共享电池配置}
...
这组配置并不对称。C 型载重最大（$80\,\text{kg}$），但只有 2 架飞机、4 组电池，而且一组亏电电池要充 50 分钟；A 型载重最小（$25\,\text{kg}$），却有 4 架飞机、6 组电池，充满只需 30 分钟。这个不对称是后文“多飞架次反而更快”现象的根源：如果重载任务全部压在 C 型上，基地会在同一时段既缺飞机又缺电池；把一部分货分给 A/B 型承担，整个机队反而能更快跑完。

\subsubsection{6.1.3 物资时限的分类与目标次序}
逐箱清单里有两个时间字段：医疗物资有“期望送达时间”，另有 30 件货箱标注了“首批保障”并给出“首批截止时间”。按题面，医疗物资的期望送达时间与首批保障货箱的截止时间都按硬约束处理，其余货箱的期望送达时间作为衡量及时性的软指标。具体到本数据：

\begin{itemize}[leftmargin=*]
    \item 医疗物资 16 件，全部带期望送达时间，构成 16 项硬约束；其中 15 件同时是首批保障箱，另加 15 项首批截止硬约束，这类合计 31 项。
    \item 首批保障箱共 30 件，每个服务区 2 件（1 件医疗物资、1 件饮用水）。除已计入上述 31 项的 15 件医疗箱外，另有 15 件饮用水箱带首批截止时间，构成 15 项硬约束。
...
\begin{align}
\text{第一阶段（主目标）：} \quad & \min \quad C_{\max} = \max_{p \in \mathcal{P}} r_p \notag \\
\text{第二阶段（次目标）：} \quad & \min \quad \sum_{b \notin \text{医疗物资}} w_b \max\left(0, t_b - D_b^{exp}\right) \notag \\
& \text{s.t.} \quad C_{\max} \le C_{\max}^*, \quad \text{以及 6.2.3 节的全部约束}
\end{align}

式中 $C_{\max}^*$ 为第一阶段求得的最小完工时间。之所以把完工时间放在第一位，是因为灾区救援的时效性主要由“最后一件物资什么时候送到”决定；医疗与首批保障箱的时限已经作为硬约束进入可行域，不需要再进目标函数。总能耗与总架次数不进入优化目标，只作为方案比较时的报告指标。

\subsection{6.2 多点多架次协同排程模型}

\subsubsection{6.2.1 航路剖面与载荷递减}
...
巡航海拔的确定需要一个高程采样规则。若沿航线按固定步长（例如不超过 $30\,\text{m}$）等间距取点，当航线恰好斜切一处山脊或只擦过某个像元的角点时，有可能漏掉该像元，从而低估沿线最高地面高程，使巡航海拔定得偏低。为此，本文把航段视为经纬度平面上的线段，求出它与 DEM 栅格网格的全部交点，再按交点与区间中点定位线段经过的每一个像元（像元按闭矩形处理，压边与过角都算经过）。设经过的像元集合为 $\mathcal{C}_{v_k, v_{k+1}}$，则

\begin{equation}
H_{v_k, v_{k+1}}^{\max} = \max_{c \in \mathcal{C}_{v_k, v_{k+1}}} \text{DEM}(c), \qquad H_{pk}^{cruise} = H_{v_k, v_{k+1}}^{\max} + 50\,\text{m}
\end{equation}

这一改动只影响个别掠角航段。例如 $O01 \to S007$，等距采样得到的沿线最高地面为 $451.83\,\text{m}$（对应巡航海拔 $501.83\,\text{m}$），而闭矩形相交法捕获到航线斜切的一处山脊，沿线最高地面为 $461.97\,\text{m}$，巡航海拔相应提高至 $511.97\,\text{m}$。两种方法给出的净空都满足 $50\,\text{m}$，差别在于后者不会遗漏被航线触碰的像元。需要说明的是，这只保证“不遗漏已采样到的像元”，净空能否满足仍然取决于 DEM 本身的精度。

航段两端的作业高度按题面取：$O01$ 为基地地面海拔，服务区 $v_k$ 为地面高程加 $30\,\text{m}$。连续访问多个服务区时，在 $v_k$ 完成交付后从 $H_{v_k}^{work}$ 重新爬升至下一段巡航海拔，爬升高度为 $h_{pk}^+ = \max(0, H_{pk}^{cruise} - H_{v_k}^{work})$。

航段飞行时间与能耗仍沿用问题一的模型，只是载荷换成该航段对应的剩余载荷：
...
架次 $p$ 的飞行总耗时与总能耗为 $T_p^{flight} = \sum_{k=0}^{K_p} t_{pk}^{flight}$，$E_p = \sum_{k=0}^{K_p} (E_{pk}^{hor} + E_{pk}^{up})$。

\subsubsection{6.2.2 电池两阶段充电模型}
架次 $p$ 返航后，电池剩余荷电状态为 $\text{SOC}_p^{ret} = 1 - E_p / E_{g(p)}^{use}$。按附录 2，电池再次投入任务前必须充至 $100\%$，且充电时间按两阶段折算：$0\sim90\%$ 阶段占等效完全充电时间 $T_{full,g}$ 的 $65\%$，$90\%\sim100\%$ 阶段占 $35\%$，各段按 SOC 增量线性折算。由此得到从 $\text{SOC}_p^{ret}$ 充满所需的时长：
...
以执行 S001 的 Q23-003 为例，C 型电池返航 SOC 为 $69.85\%$，代入式 (\ref{eq:charge_model}) 得 $c = 3000 \times (0.65 \times 0.2015/0.90 + 0.35) = 1486.6\,\text{s}$，与求解输出的充电时长一致。

\subsubsection{6.2.3 决策变量与约束}
模型引入以下变量：$y_{pb} \in \{0,1\}$ 表示货箱 $b$ 是否由架次 $p$ 承运；$x_{pg} \in \{0,1\}$ 表示架次 $p$ 是否选用机型 $g$；$z_{pu} \in \{0,1\}$ 表示架次 $p$ 是否指派实体机 $u$；$\theta_{pk} \in \{0,1\}$ 表示架次 $p$ 是否指派电池 $k$；$s_p \ge 0$ 为架次 $p$ 的准备开始时刻；$r_p \ge 0$ 为架次 $p$ 返抵 $O01$ 的时刻。

约束包括以下几组。

\begin{enumerate}[leftmargin=*]
    \item \textbf{货箱唯一覆盖}。全部 80 件货箱必须且仅被配送一次，且只能送至所属服务区：
    \begin{equation}
    \sum_{p \in \mathcal{P}} y_{pb} = 1, \quad \forall b \in \mathcal{B}
    \end{equation}

    \item \textbf{载重与容积}。架次所载物资的总质量与总体积不超过所选机型的结构上限：
    \begin{equation}
    ...
    \item \textbf{返航安全余量}。架次返航时剩余电量不低于 $\rho = 20\%$：
    \begin{equation}
    E_p \le (1 - \rho) \sum_{g \in \mathcal{G}} E_g^{use} x_{pg} \iff \text{SOC}_p^{ret} \ge 20\%, \quad \forall p \in \mathcal{P}
    \end{equation}
    这里 $E_p$ 由 6.2.1 节的逐段载荷递减模型给出，即多点架次在每个航段都按当时的剩余载荷重新计算能耗。

    \item \textbf{时序传递与交付时刻}。架次 $p$ 的准备耗时为 $\tau_p^{prep} = \sum_g \tau_g^{prep} x_{pg}$，装载耗时按箱数计。起飞时刻为 $t_p^{takeoff} = s_p + \tau_p^{prep} + \sum_{b \in \mathcal{B}_p} \tau_{g(p)}^{load}$（本文按准备与装载串行处理）。在服务区 $v_k$ 完成交付的绝对时刻为
    \begin{equation}
    ...
    \end{equation}
    返抵 $O01$ 的时刻为 $r_p = t_{p, v_{K_p}}^{deliver} + t_{p, K_p}^{flight}$。

    \item \textbf{硬时限}。医疗物资与首批保障货箱必须在截止时刻前完成交付：
    \begin{equation}
    t_{p, \text{site}(b)}^{deliver} \le D_b^{hard}, \quad \forall b \in \mathcal{B}_p \cap \mathcal{B}_{hard}, \quad \forall p \in \mathcal{P}
    \end{equation}

    \item \textbf{实体机串行}。同一架实体机在同一时刻只能被一个架次占用，占用区间为 $[s_p, r_p)$：
    \begin{equation}
    \left( z_{pu} = 1 \land z_{p'u} = 1 \right) \implies \left[ s_p, r_p \right) \cap \left[ s_{p'}, r_{p'} \right) = \emptyset, \quad \forall u \in \mathcal{U}, \; p \ne p'
    \label{eq:drone_no_overlap}
    \end{equation}

    \item \textbf{电池闭环无重叠}。电池在准备开始时刻 $s_p$ 装机占用，飞行至 $r_p$ 返航，随后立即充电 $c_p$ 秒，占用周期为 $[s_p, r_p + c_p)$，其间不可被其他架次使用：
    \begin{equation}
    \left( \theta_{pk} = 1 \land \theta_{p'k} = 1 \right) \implies \left[ s_p, r_p + c_p \right) \cap \left[ s_{p'}, r_{p'} + c_{p'} \right) = \emptyset, \quad \forall k \in \mathcal{K}, \; p \ne p'
    \label{eq:battery_no_overlap}
    \end{equation}
    此外，实体机与电池的机型必须与架次机型一致：$z_{pu} = 1 \implies \text{aircraft}(u) = g(p)$，$\theta_{pk} = 1 \implies \text{aircraft}(k) = g(p)$。
\end{enumerate}

\subsection{6.3 求解算法}

\subsubsection{6.3.1 两阶段分解}
上述模型是带容量、能量、时限、多机型和电池周转的车辆路径问题，属于强 NP-hard\cite{schermer2019}。若把 80 件不可拆分货箱、15 个节点、8 架实体机与 14 组电池放在连续时间域上做一体化建模，状态空间过大，求解器很难在可接受时间内收敛。

本文采用“先生成候选组批拓扑、再对固定拓扑做精确排程”的两阶段分解：

\begin{enumerate}[leftmargin=*]
    \item \textbf{逐区精确集合划分}。沿用问题一的状态压缩动态规划，把每个服务区作为独立单元枚举满足载重、容积与能量约束的货箱子集，分别按“架次优先”“时间优先”“能耗优先”三种字典序求出该区的精确最优划分。得到两个基准单点组批：18 架次（架次优先与时间优先重合）与 19 架次（能耗优先）。另用一种构造式贪心装配（每次优先装硬时限货箱，在可行组合中选飞行与地面时间最短者，并给稀缺的 C 型机加惩罚权重）生成 31 架次的轻量单点划分，作为对照。
    \item \textbf{两区邻域合并}。在单点组批基础上遍历服务区对 $(S_i, S_j)$，若两个单点架次机型相同、服务区不重叠，且合并后总质量、总体积不超限、往返各段能耗与返航 SOC 均满足约束、合并航线下各点仍能满足硬时限，则生成两点回路 $O01 \to S_i \to S_j \to O01$ 候选。该步骤把 31 架次的构造方案压缩为 25 架次的多点方案，合并出的 6 条回路为 S013$\to$S010、S010$\to$S014、S007$\to$S015、S013$\to$S009、S009$\to$S012、S006$\to$S007。
\end{enumerate}

\subsubsection{6.3.2 CP-SAT 精确排程}
对每一种固定的组批拓扑，用 Google OR-Tools 的 CP-SAT 求解器做资源和时间的联合排程。CP-SAT 结合了约束传播、SAT 求解与分支定界，适合处理区间不重叠（\texttt{NoOverlap}）这类约束\cite{laborie2018}。实现上有三点需要注意。

\begin{enumerate}[leftmargin=*]
    \item \textbf{整秒网格}。模型中的时间统一取整秒：航段时间与充电时间向上取整，硬时限向下取整。这样在整数网格上求得的解，映射回连续时间的浮点模型后一定仍然满足硬时限与不重叠要求。
    \item \textbf{可选区间变量}。对架次 $p$ 的每一种可行机型与实体机组合，建立可选区间 $\text{interval}_{pu} = [s_p,\, s_p + d_p)$，布尔标志为 $z_{pu}$；对每组可用电池建立含充电周期的区间 $\text{interval}_{pk}^{batt} = [s_p,\, s_p + d_p + c_p)$，标志为 $\theta_{pk}$，其中 $d_p$ 为架次时长、$c_p$ 为充电时长。分别对每架实体机和每组电池的区间集合调用 \texttt{AddNoOverlap}。
    \item \textbf{两级目标}。先以完工时间 $C_{\max} = \max_p r_p$ 为目标求解，得到 $C_{\max}^*$ 及其下界；再把 $C_{\max} \le C_{\max}^*$ 固定下来，以非医疗货箱的加权迟到量为目标做第二次优化。最终解再由独立的浮点审计模块逐航段重放复核。
\end{enumerate}

有一点需要说明：由于同一条航线的货箱组合可能对多种机型都可行，CP-SAT 会在排程时重新为每条航线选择机型。因此同一组批拓扑在不同排程方法下的总能耗可能不同——这一点在表 \ref{tab:candidates_comparison_q2} 中可以观察到，例如 19 架次组批在 CP-SAT 下总能耗为 $61.314\,\text{kWh}$，在两种启发式规则下为 $60.566\,\text{kWh}$。表中的能耗是排程结果对应的能耗，不是组批阶段的标称能耗。

算法流程见算法 \ref{alg:cpsat_scheduler}。
...
\Ensure 协同调度方案（架次时刻、机体与电池分配、完工时间、加权迟到量）
\State 建立 CP-SAT 模型 $M$
\For{每条航线 $p \in \mathcal{P}$}
    \State 枚举该航线全部可行机型，得到选项集合 $\text{Options}_p$
    \State 创建整数变量 $s_p \in [0, H]$ 与机型指示变量 $x_{pg}$，约束 $\sum_{g} x_{pg} = 1$
    \For{每个机型选项 $g \in \text{Options}_p$}
        \For{同机型实体机 $u \in \mathcal{U}_g$}
            \State 创建机体区间 $I_{pu}^{drone} = [s_p, s_p + d_{pg})$，标志 $z_{pu}$，加入 $\mathcal{L}_u^{drone}$
        \EndFor
        \For{同机型电池 $k \in \mathcal{K}_g$}
            \State 计算充电时长 $c_{pg}$，创建电池区间 $I_{pk}^{batt} = [s_p, s_p + d_{pg} + c_{pg})$，标志 $\theta_{pk}$，加入 $\mathcal{L}_k^{batt}$
        \EndFor
        \State 对每个硬时限货箱 $b$，添加 $s_p + \Delta t_{pg,b} \le \lfloor D_b^{hard} \rfloor$
    \EndFor
\EndFor
\For{每架实体机 $u$}
    \State $M.\text{AddNoOverlap}(\mathcal{L}_u^{drone})$
\EndFor
\For{每组电池 $k$}
    \State $M.\text{AddNoOverlap}(\mathcal{L}_k^{batt})$
\EndFor
\State 定义 $C_{\max} \ge r_p \; (\forall p)$，令 $M.\text{Minimize}(C_{\max})$，求解得 $C_{\max}^*$ 与下界
\State 添加 $C_{\max} \le C_{\max}^*$，令 $M.\text{Minimize}(\sum_b w_b \cdot \text{Late}_b)$，再求解一次
\State 将整秒解映射回浮点模型，由独立审计模块逐航段复核
\State \Return 审计通过的最优协同调度方案
\end{algorithmic}
\end{algorithm}

\subsection{6.4 结果与方案比较}

\subsubsection{6.4.1 候选方案对比}
对 4 种组批拓扑（架次优先、能耗优先、构造式单点、构造式跨区合并）分别用 CP-SAT 和两种确定性启发式排程规则（平衡、节能）求解，共得到 12 组可行方案，指标见表 \ref{tab:candidates_comparison_q2}。

\begin{table}[H]
\centering
\small
\caption{问题二 12 组候选调度方案指标对比}
\label{tab:candidates_comparison_q2}
\begin{tabularx}{\textwidth}{l c c c c c c >{\raggedright\arraybackslash}X}
\toprule
\textbf{组批拓扑与排程策略} & \textbf{求解方法} & \textbf{架次} & \textbf{总能耗} & \textbf{完工时间} & \textbf{加权迟到} & \textbf{准时率} & \textbf{特点} \\
& & (架次) & ($\text{kWh}$) & ($\text{s}$) & & & \\
\midrule
\multicolumn{8}{l}{\textit{一、18 架次（第一问架次优先/时间优先单点组批）}} \\
min\_sorties / cp\_sat & CP-SAT & 18 & 60.664 & 9581.4 & 362113.2 & 85.0\% & C 型电池充能排队严重 \\
min\_sorties / heu\_balanced & 启发式平衡 & 18 & 60.664 & 10657.8 & 447130.1 & 87.5\% & 串行程度高，完工时间 2.96 h \\
min\_sorties / heu\_energy & 启发式节能 & 18 & 60.664 & 10657.8 & 447130.1 & 87.5\% & 与平衡启发式同解 \\
\midrule
\multicolumn{8}{l}{\textit{二、19 架次（第一问能耗优先单点组批）}} \\
min\_energy / cp\_sat & CP-SAT & 19 & 61.314 & 8677.0 & 141886.2 & 88.8\% & 拆分 S008 后完工时间缩短 \\
min\_energy / heu\_balanced & 启发式平衡 & 19 & 60.566 & 9895.6 & 316237.9 & 87.5\% & 启发式排程时间偏长 \\
min\_energy / heu\_energy & 启发式节能 & 19 & 60.566 & 9895.6 & 316237.9 & 87.5\% & -- \\
\midrule
\multicolumn{8}{l}{\textit{三、31 架次（构造式单点组批）}} \\
constructive / cp\_sat & CP-SAT & 31 & 82.924 & 8370.9 & 281712.2 & 88.8\% & 架次过多，起降频繁且电耗大 \\
constructive / heu\_balanced & 启发式平衡 & 31 & 81.838 & 10115.3 & 114740.7 & 88.8\% & 加权迟到最低 \\
constructive / heu\_energy & 启发式节能 & 31 & 78.981 & 15298.3 & 311022.5 & 86.2\% & 为节能牺牲时间，完工拖到 4.25 h \\
\midrule
\multicolumn{8}{l}{\textit{四、25 架次（构造式跨区合并组批，主选方案）}} \\
\textbf{merged / cp\_sat} & \textbf{CP-SAT} & \textbf{25} & \textbf{74.628} & \textbf{7983.8} & \textbf{90818.0} & \textbf{90.0\%} & \textbf{完工时间最短，加权迟到最低} \\
merged / heu\_balanced & 启发式平衡 & 25 & 73.994 & 12576.7 & 313070.0 & 82.5\% & 无法协调多点航线的资源冲突 \\
merged / heu\_energy & 启发式节能 & 25 & 73.994 & 12576.7 & 313070.0 & 82.5\% & 完工时间 3.49 h \\
\bottomrule
\end{tabularx}
\end{table}

从表 \ref{tab:candidates_comparison_q2} 可以看出三点。

\begin{enumerate}[leftmargin=*]
    \item \textbf{架次最少不等于完工最快。} 18 架次方案在问题一的单点往返体制下是最优的，但放进问题二的资源排程后，完工时间长达 $9581.4\,\text{s}$。原因是该方案在单点组批阶段把重载任务主要交给 C 型承担，而基地只有 2 架 C 型实体机和 4 组电池，一组 C 型电池充满需要 $3000\,\text{s}$，这些 C 型任务在时间轴上互相等待，基地出现“飞机等电池”的排队。

    \item \textbf{适度拆解多点航线能激活机队并发。} 主选的 25 架次方案通过 6 条跨区合并航线，把一部分载荷转移给 A 型和 B 型承担。架次数由 18 增至 25，总能耗由 $60.664\,\text{kWh}$ 升至 $74.628\,\text{kWh}$，但 4 架 A 型（6 组电池、充能 30 分钟）和 2 架 B 型的周转能力被充分利用，C 型电池的充能等待不再是瓶颈。完工时间由 $9581.4\,\text{s}$ 降到 $7983.8\,\text{s}$，缩短 $1597.6\,\text{s}$，降幅 $16.7\%$。

    \item \textbf{时效指标同步改善。} 主选方案的非医疗货箱加权迟到量由 18 架次方案的 $362113.2$ 降到 $90818.0$，降幅 $74.9\%$；全部 80 件货箱的期望时间准时率由 $85.0\%$ 升至 $90.0\%$；46 项硬时限全部按时达标。
\end{enumerate}

需要指出，表 \ref{tab:candidates_comparison_q2} 中的 12 组方案都是在给定组批拓扑下求得的可行解，CP-SAT 给出的是固定拓扑下的最优排程，并不构成问题二的全局最优性证明。

\subsubsection{6.4.2 主选方案明细}
主选方案共 25 个架次，80 件货箱全部交付。各架次的执行飞机、电池、时刻、航线、装载质量、能耗与返航电量见表 \ref{tab:sorties_detail_q2}。

\begin{table}[H]
\centering
\scriptsize
\caption{问题二主选方案（25 架次）排程与资源周转明细}
...
\midrule
\textbf{全网汇总} & \textbf{8 架全部出动} & \textbf{8C+7B+10A} & \textbf{13 组周转} & -- & -- & \textbf{7983.8} & \textbf{含 6 条多点回路} & \textbf{758.0} & \textbf{74.628} & \textbf{均值 48.2\%} \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{6.4.3 图表解读}
图 \ref{fig:routes_q2}、图 \ref{fig:clearance_q2}、图 \ref{fig:resource_gantt_q2}、图 \ref{fig:delivery_times_q2} 与图 \ref{fig:tradeoff_q2} 分别给出航线空间布局、净空核查、资源甘特图、交付时刻分布与多目标权衡。
...
由图 \ref{fig:routes_q2} 可见，航线呈“近点直达、外围串联”的布局：物资量最大的 S001（$154\,\text{kg}$）用 3 个架次直达；S010、S013、S014、S007、S015、S009、S012 等外围轻量服务区被编成 6 条双点回路（Q23-020$\sim$Q23-025），一个架次顺路完成两个点，减少了重复往返 $O01$ 的空驶里程。

\begin{figure}[H]
\centering
\SafeGraphic[width=0.88\textwidth]{terrain_clearance.png}
\caption{各飞行航段最高地面高程与计划巡航高度对比（50 m 安全净空核查）}
\label{fig:clearance_q2}
\end{figure}

图 \ref{fig:clearance_q2} 给出 25 个架次执行的 56 个飞行航段（35 条不同有向航段）的最高地面高程与计划巡航高度。全部航段的净空都在 $50\,\text{m}$ 及以上。其中沿线地面最高的是 $O01 \leftrightarrow S014$ 航段（$541.95\,\text{m}$，巡航海拔 $591.95\,\text{m}$）与 $S006 \to S007$ 航段（$540.36\,\text{m}$，巡航海拔 $590.36\,\text{m}$）；$O01 \to S007$ 经闭矩形相交修正后沿线地面为 $461.97\,\text{m}$，巡航海拔 $511.97\,\text{m}$。
...
图 \ref{fig:resource_gantt_q2} 呈现了 8 架实体机与 14 组电池的时间轴。有两点值得注意：

\begin{itemize}[leftmargin=*]
    \item \textbf{机队利用率并不均衡。} 除 U03 外，其余 7 架在 $t=0$ 即开始地面准备，最晚的到 $540\,\text{s}$ 升空。全程 U07、U08 各执行 4 个架次，U01、U02、U04、U05、U06 各 3 个，而 U03 只在 $2655\,\text{s}$ 执行了 1 个架次（Q23-012）。从电池侧看，A 型 6 组电池中有 5 组被使用，A-B04 全程未参与周转——对 A 型机队来说，真正的限制来自电池而不是飞机。
    \item \textbf{电池周转按充电节拍错开。} 以 C-B02 为例，它在 $0\sim1363.4\,\text{s}$ 执行 Q23-003，返航 SOC 为 $69.85\%$，随后立即接入充电，按式 (\ref{eq:charge_model}) 需 $1486.6\,\text{s}$，于 $2850.0\,\text{s}$ 充满；但下一次使用要到 $3436.0\,\text{s}$（Q23-005），中间的空档是飞机排期决定的。C-B01、C-B03、C-B04 也都完成了 2 次循环，各电池的占用与充电区间互不重叠。
\end{itemize}

\begin{figure}[H]
\centering
\SafeGraphic[width=0.88\textwidth]{delivery_times.png}
\caption{问题二 80 件应急物资实际交付时刻与时限门槛对比图}
\label{fig:delivery_times_q2}
\end{figure}

由图 \ref{fig:delivery_times_q2} 可以核对时限达标情况：实际交付时刻均落在对应硬截止线左侧，46 项硬约束无违约；80 件物资中 72 件在期望时间内送达，8 件延迟的全部是后期批次的饮用水，延迟量受完工时间约束，不会无限扩大。

\begin{figure}[H]
\centering
\SafeGraphic[width=0.82\textwidth]{time_priority_tradeoff.png}
\caption{问题二候选方案在完工时间、出动架次与加权迟到量之间的权衡散点图}
\label{fig:tradeoff_q2}
\end{figure}

图 \ref{fig:tradeoff_q2} 给出候选方案在三个指标上的分布。主选的 \texttt{merged/cp\_sat} 方案在完工时间轴上取得最小值 $7983.8\,\text{s}$，加权迟到量也处于最低档，因此在所比较的方案中占据明显优势。

\subsection{6.5 资源瓶颈分析与可行性审计}

\subsubsection{6.5.1 瓶颈的时间演化}
把甘特图按时间切片，可以看到瓶颈在两小时内发生了转移。

\begin{enumerate}[leftmargin=*]
    \item \textbf{$0\sim1630\,\text{s}$：受飞机和地面作业限制。} 14 组电池初始全部满电，能同时出动的架次数只取决于飞机数量与地面保障速度。这一阶段 7 架飞机在 $t=0$ 同时开始准备，到 $540\,\text{s}$ 全部升空，主要的约束是装载与飞行时间。
    \item \textbf{$1630\sim2850\,\text{s}$：受 C 型电池限制。} C-B02、C-B01 分别在 $1363.4\,\text{s}$ 和 $1627.4\,\text{s}$ 返航后进入充电，C-B03、C-B04 此时在空中，四组 C 型电池全部处于占用或充电状态，没有一组可用，新的 C 型架次排不进来。这一时段的投送任务因此主要由 A/B 型承担——Q23-010、Q23-018、Q23-019、Q23-022、Q23-023 等架次都排在 $2000\sim2700\,\text{s}$ 之间。
    \item \textbf{$2850\,\text{s}$ 之后：瓶颈解除，进入收尾。} C-B02 在 $2850.0\,\text{s}$、C-B01 在 $3274.5\,\text{s}$ 先后充满，C 型恢复周转能力。U07、U08 随后各再执行 2 个架次，最后一个架次 Q23-007 于 $7983.8\,\text{s}$ 返航，全部任务结束。
\end{enumerate}

从整个过程看，完工时间最终由 C 型机队决定：U07 与 U08 的占用分别持续到 $7964.2\,\text{s}$ 和 $7983.8\,\text{s}$，是全部飞机中结束最晚的两架（次晚的 U06 为 $7752.8\,\text{s}$，最早的 U03 为 $3967.6\,\text{s}$）。

\subsubsection{6.5.2 方案可行性审计}
为核对排程方案是否真的满足全部物理与资源约束，本文另写了一个独立于求解算法的审计模块（输出为 \texttt{validation.json}），对 25 个架次的每个物理量重新计算。审计结果如下。

\begin{itemize}[leftmargin=*]
    \item \textbf{航路净空}：用独立工具（\texttt{terrain\_audit.py}）重新按线段与栅格闭矩形求交，不调用求解器的采样函数。对 56 个飞行航段（35 条有向航段）逐一核验，巡航海拔与沿线最高地面高程之差的最小值为 $50.000\,\text{m}$，违规航段数为 0。
    \item \textbf{货箱覆盖}：80 件货箱唯一映射，分配率 $100\%$，无重复、无遗漏。
    \item \textbf{硬时限}：46 项硬约束合规率 $100\%$，违约数 0。
    \item \textbf{动力学与电量}：25 个架次的返航 SOC 在 $23.15\%\sim75.81\%$ 之间（最低 Q23-007 的 $23.15\%$，最高 Q23-015 的 $75.81\%$），均高于 $20\%$ 下限。
    \item \textbf{实体机无重叠}：8 架飞机的占用区间两两无交集，式 (\ref{eq:drone_no_overlap}) 满足。
    \item \textbf{电池无重叠}：14 组电池的 $[s_p, r_p + c_p)$ 占用区间两两无交集，式 (\ref{eq:battery_no_overlap}) 满足。
\end{itemize}

\subsubsection{6.5.3 最优性说明}
需要说明本文解的“最优”是在什么范围内成立的。在所选定的组批拓扑下，CP-SAT 对整秒离散模型求得了 \texttt{OPTIMAL} 状态：目标值（整秒完工时间）为 $7984\,\text{s}$，求解器给出的下界同样是 $7984\,\text{s}$，两者重合；把该解映射回浮点模型后，实际完工时刻为 $7983.81\,\text{s}$。因此这是\textbf{固定组批拓扑、整秒时间网格下的条件最优解}，并不是未加拓扑限制时问题二的全局最优性证明。

若要进一步给出全局最优性证书，可以在分支定价（Branch-and-Price）框架下做列生成：把连续的多点航线生成作为带资源约束的最短路子问题（ESPPRC），在主问题与子问题之间迭代生成列，从而得到全局对偶下界与最优性间隙。本文没有做到这一步，但就应急场景而言，在数十秒内给出一个有完整可行性审计背书的 $7983.8\,\text{s}$ 方案，已经具备实用价值。
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{只在固定组批拓扑内保证最优。} 逐区组批用的是精确动态规划，但两区邻域合并是贪心过程；CP-SAT 的 \texttt{OPTIMAL} 也只对给定的拓扑成立。因此 25 架次方案是“本文候选拓扑中最好的方案”，不是问题二的全局最优解。要给出全局最优性证书需要列生成一类的精确方法，本文没有做到。
    \item \textbf{排程阶段会重新选择机型。} 同一条航线的货箱组合往往对多种机型都可行，CP-SAT 在排程时按完工时间重新选型，所以同一组批在不同排程方法下能耗不同（见表 \ref{tab:candidates_comparison_q2}）。表中能耗是排程解的能耗，与组批阶段的标称能耗不能混用。
    \item \textbf{软迟到量的统计口径。} 本文按求解器的实现，对全部 64 件非医疗货箱计算加权迟到量；若改为只统计“既非医疗也非首批保障”的货箱（49 件），数值会不同。两种口径都能自洽，但论文中必须写明用的是哪一种。
